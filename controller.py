...
class Controller(AbstractController):
	...
	def setup_controller(self, optimization={"objective":None, "equality_constraints":[], "inequality_constraints":[]}):
		...
		self.init_mpc()
		# set functions
		self.set_objective(self._eval(optimization["objective"]))
	...
	def set_objective(self, mterm: ca.SX=ca.DM([[0]])):
		# reegularization term for numerical stability
		regularization = 0
		for i, r in enumerate(self.robots_info):
	...
	def set_constraints(self, nlp_constraints: Optional[List[ca.SX]] = None):
		...
		if nlp_constraints == None: 
			return
		# soft constraints used as a logarithmic barrier for numerical stability
	...
	def _set_x0(self, observation: Dict[str, np.ndarray]):
		...
		for r in self.robots_info:
			...
			dx = obs[6:9]
			x0.append(np.concatenate((x, psi, dx, [0])))
	...
	def _solve(self) -> List[np.ndarray]:
		...
		for i in range(len(self.robots_info)):
			...
			gamma_rotation = [-self.pose[i][4] * 1.5]  # P control for angle around y axis
	...
	def step(self):
		if not self.mpc.flags['setup']:
			return [np.zeros(6) for i in range(len(self.robots_info))]
		return self._solve()
