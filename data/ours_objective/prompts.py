...
You must format your response into a json. Here are some examples:

objects = ['object_1', 'object_2']
# Query: move the gripper to [0.2, 0.05, 0.2]
{
...
objects = ['apple', 'pear']
# Query: move the gripper to apple
{
...
# Query: Move the gripper behind fork
{
...
Rules:
  (1) Use these common sense rules for spatial reasoning:
    (a) 'in front of' and 'behind' for positive and negative x-axis directions.
    (b) 'to the left' and 'to the right' for positive and negative y-axis directions.
    (c) 'above' for positive z-axis directions.
...
# Query: put the coffee pod into the coffee machine
{
  "tasks": ["left robot: move gripper above the coffee pod 1. right robot: move gripper above the coffee pod 2 and avoid collisions with the coffee machine", "left robot: move gripper above the coffee pod 1. right robot: move gripper above the coffee pod 2 and avoid collisions with the coffee machine", "left robot: close_gripper(). right robot: close_gripper()", "left robot: move the gripper above the coffee machine. right robot: do nothing", "left robot: open_gripper(). right robot: do nothing", "left robot: move the gripper behind the coffee machine. right robot: move the robot above the coffee machine". "left robot: do nothing. right robot: open_gripper()"]
}

objects = ['rope left', 'rope right', 'rope']
# Query: move the rope 0.1m to the left
{
  "tasks": ["left robot: move gripper above rope left. right robot: move gripper above rope right", "left robot: move gripper to rope left. right robot: move gripper to rope right", "left robot: close_gripper(). right robot: close_gripper()", "left robot: move gripper 0.1m to the left. right robot: move gripper 0.1m to the left.", "left robot: open_gripper(). right robot: open_gripper()"]
}

objects = ['apple', 'drawer handle', 'drawer']
# Query: put apple into the drawer.
{
  "tasks": ["left robot: move gripper to drawer handle. right robot: move gripper above the apple", "left robot: do nothing. right robot: move gripper to the apple" "left robot: close_gripper(). right robot: close gripper.", "left robot: move gripper 0.25m in the y direction. right robot: do nothing.", "left robot: do nothing. right robot: move gripper above the drawer.", "left robot: do nothing. right robot: open_gripper()"]
}
"""
...
objects = ['object_1', 'object_2']
# Query: left robot: move the gripper to [0.2, 0.05, 0.2]. right robot: do nothing.
{
...
objects = ['red_cube', 'yellow_cube']
# Query: left robot: move the gripper above red cube. right robot: move the gripper above yellow cube.
{
  "objective": "ca.norm_2(x_left - (red_cube.position + np.array([0, 0, red_cube.size])))**2 + ca.norm_2(x_right - (yellow_cube.position + np.array([0, 0, yellow_cube.size])))**2"
}
...
objects = ['coffee_pod', 'coffee_machine']
# Query: left robot: move gripper above the coffe pod. right robot: move gripper above the coffe machine.
{
...
objects = ['joystick', 'remote']
# Query: left robot: Move the gripper at constant speed along the x axis. right robot: Move the gripper at constant speed along the x axis.
{
